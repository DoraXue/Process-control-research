...
%     
%     % calculate alpha
%     Q = -((C*Amodel*C^(-1) - C*Bmodel*K)'*P + P*(C*Amodel*C^(-1) - C*Bmodel*K));
%     alpha(1,loc) = min(eig(Q));
%     
%     % no deltaB
%     den = 2*(norm(P)*norm(C)*norm(Bmodel)*norm(K) + norm(deltaB)*norm(P)*norm(C)*norm(K));
%     num = alpha(1,loc)/2 - 2*norm(P*C)*norm(deltaA)*norm(C^(-1));
%     Thresh(1,loc) = num/den;
%     
%     % with deltaB
% %     den = 2*(norm(P*C*Bmodel*K) + norm(deltaB)*norm(P*C)*norm(K));
% %     num = alpha(1,loc)/2 - 2*norm(P*C)*(norm(deltaA)*norm(C^(-1) + norm(deltaB)*norm(K)));
% %     Thresh(1,loc) = num/den;
% %     
%     
% end
...
%        SenPos = ActPos;
    ...
    if rank(Co) == 3
        ...
        ControllableRegion(z2,z3) = 0;      % uncontrollable
    end
    ...
    % calculate alpha
    QQ = -((C*Amodel*C^(-1) - C*Bmodel*K)'*P + P*(C*Amodel*C^(-1) - C*Bmodel*K));
    alpha(z2,z3) = min(eig(QQ));
    
    % no deltaB
    den = 2*(norm(P)*norm(C)*norm(Bmodel)*norm(K) + norm(deltaB)*norm(P)*norm(C)*norm(K));
    num = alpha(z2,z3)/2 - 2*norm(P*C)*norm(deltaA)*norm(C^(-1));
    Thresh(z2,z3) = num/den;
%     
    % with deltaB
%     den = 2*(norm(P*C*Bmodel*K) + norm(deltaB)*norm(P*C)*norm(K));
%     num = alpha(z2,z3)/2 - 2*norm(P*C)*(norm(deltaA)*norm(C^(-1) + norm(deltaB)*norm(K)));
%     Thresh(z2,z3) = num/den;
   end
end
...
xlabel('Z_a_2','FontSize',30);
ylabel('Z_a_3','FontSize',30)
